...
bool beogh_can_gift_items_to(const monster* mons, bool quiet)
{
    ...
    // Maybe useless. But someday, one can revive this.
    if (given_gift(mons))
    {
        if (!quiet)
        {
...
bool gozag_call_merchant()
{
    ...
    for (int i = 0; i < NUM_SHOPS; i++)
    {
        ...
        if (type == SHOP_FOOD)
            continue;
        if (type == SHOP_DISTILLERY && (you.species == SP_MUMMY || you.species == SP_LICH ||  you.species == SP_WIGHT))
            continue;
...
static mutation_type _random_valid_sacrifice(const vector<mutation_type> &muts)
{
    ...
    for (auto mut : muts)
    {
        ...
        if (mut == MUT_NO_DEVICE_HEAL && you.species == SP_MUMMY || you.species == SP_LICH ||  you.species == SP_WIGHT)
            continue;
