...
static bool _foe_near_wall(const monster& caster);
static bool _foe_not_nearby(const monster& caster);
                     ...
                     bool check_validity)
{
    ...
    else if (spell_cast == SPELL_EVAPORATE)
    {
        int cloud = random2(5);
        switch (cloud)
        {
...
bool handle_mon_spell(monster* mons)
{
    ...
    if (flags & MON_SPELL_ANTIMAGIC_MASK && ((mons->has_ench(ENCH_ANTIMAGIC)
        ...
        (mons->antimagic_haloed() && coinflip())))
    {
