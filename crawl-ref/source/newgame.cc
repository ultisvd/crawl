...
static job_group jobs_order[] =
{
    ...
    },
    {
        "Zealot",
        coord_def(1, 0), 25,
        { JOB_BERSERKER, JOB_ABYSSAL_KNIGHT, JOB_CHAOS_KNIGHT, JOB_MELTED_KNIGHT }
    },
    {
        "Warrior-mage",
        coord_def(1, 5), 26,
        { JOB_SKALD, JOB_TRANSMUTER, JOB_WARPER, JOB_ARCANE_MARKSMAN,
          JOB_ENCHANTER, JOB_STALKER, JOB_CRUSADER }
    },
    {
        "Mage",
        coord_def(2, 0), 22,
                           ...
                           const newgame_def& defaults)
{
    ...
    if (ng.species == SP_FELID || ng.species == SP_HYDRA)
        return true;
...
static bool _choose_job_specific(newgame_def& ng, newgame_def& ng_choice)
{
    if (ng.job != JOB_MELTED_KNIGHT 
        && ng.job != JOB_ICE_ELEMENTALIST 
        && ng.job != JOB_CARAVAN 
        && ng.job != JOB_COLLECTOR
        && !(ng.job == JOB_ARTIFICER && (ng.species != SP_FELID 
                                      && ng.species != SP_HYDRA
                                      && ng.species != SP_CRUSTACEAN)))
        return true;
